...
CREATE POLICY "Admins can delete all addresses" ON public.addresses
  FOR DELETE USING (public.is_admin(auth.uid()));
...
CREATE POLICY "Admins can delete all payment methods" ON public.payment_methods
  FOR DELETE USING (public.is_admin(auth.uid()));
...
-- Function to create a new user with auth and profile
...
RETURNS TABLE(user_id UUID, error TEXT) AS $$
DECLARE
  new_user_id UUID;
BEGIN
  -- Create auth user
  -- Note: In a real implementation, you would use Supabase auth admin APIs
  -- This is a simplified version for demonstration
  
  -- Insert profile
  INSERT INTO public.profiles (id, email, full_name, role, created_at, updated_at)
  VALUES (gen_random_uuid(), email, full_name, role, NOW(), NOW())
  RETURNING id INTO new_user_id;
  
  RETURN QUERY SELECT new_user_id, NULL::TEXT;
EXCEPTION
  WHEN unique_violation THEN
    RETURN QUERY SELECT NULL::UUID, 'Email already exists'::TEXT;
  WHEN OTHERS THEN
...
BEGIN
  ...
  DELETE FROM public.payment_methods WHERE user_id = user_id;
  DELETE FROM public.addresses WHERE user_id = user_id;
  DELETE FROM public.user_security_settings WHERE user_id = user_id;
  DELETE FROM public.user_preferences WHERE user_id = user_id;
  
  -- Delete profile (this will cascade to auth.users)
